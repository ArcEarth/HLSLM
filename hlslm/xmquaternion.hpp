...
namespace DirectX
{
	namespace hlsl
	{
		...
		public:
			...
			inline const xmvector<Scalar,3>& axis() const{
				return this->as<3>();
			}

			xmscalar<Scalar> angle() const {
				return 2.0f * acos(base_type(detail::splat<3>(this->v)));
			}

			template <unsigned _X, unsigned _Y, unsigned _Z>
			static inline xmvector<Scalar,3> euler_abc() const
			{
				DirectX::XMQuaternionEulerAngleABC<_X,_Y,_Z>(this->v);
			}

			static inline xmvector<Scalar,3> euler_yaw_pitch_roll() const
			{
				DirectX::XMQuaternionEulerAngleABC<_X,_Y,_Z>(this->v);
			}
